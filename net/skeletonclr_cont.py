...
class SkeletonCLR(nn.Module):
    ...
    def forward(self, im_q, im_k=None, im_m=None, label=None, topk=1, weight=False):
        """
        Input:
        ...
        l_pos_m = torch.einsum('nc,nc->n', [m, k]).unsqueeze(-1)
        l_neg_m = torch.einsum('nc,ck->nk', [m, self.queue.clone().detach()])
        ...
        topk_onehot_neg_1 = torch.zeros_like(l_neg_m)

        pos_mask = torch.cat([torch.ones(topk_onehot.size(0), 1).cuda(), topk_onehot], dim=1)
        pos_mask_m = torch.cat([torch.ones(topk_onehot_neg_1.size(0), 1).cuda(), topk_onehot_neg_1], dim=1)
        target_mask = label.clone().unsqueeze(-1) == self.queue_label.clone().unsqueeze(0)

        # dequeue and enqueue
        self._dequeue_and_enqueue(k, label)

        return logits, pos_mask.detach(), logits_m, pos_mask_m.detach(), topk_onehot.detach(), target_mask.detach()
