...
class SkeletonCLR(nn.Module):
    ...
    def forward(self, im_q, im_k=None, label=None, topk=1, weight=False):
        ...
        pos_mask_reg1 = torch.cat([torch.ones(topk_onehot.size(0), 1).cuda(), topk_onehot_neg_1, topk_onehot_neg_2],
                                  dim=1)
        pos_mask_reg2 = torch.cat([torch.ones(topk_onehot.size(0), 1).cuda(), topk_onehot_neg_1, topk_onehot_neg_2],
                                  dim=1)
        ...
        return logits, pos_mask.detach(), logits_reg_1, pos_mask_reg1.detach(), \
               logits_reg_2, pos_mask_reg2.detach(), topk_onehot.detach(), target_mask.detach()
