...
class CrosSCLR(nn.Module):
    ...
    def cross_training(self, im_q, im_k, topk=1, context=True):
        ...
        prob, topkdix = torch.topk(l_neg, topk, dim=1)
        prob_motion, topkdix_motion = torch.topk(l_neg_motion, topk, dim=1)
        prob_bone, topkdix_bone = torch.topk(l_neg_bone, topk, dim=1)
        ...
        return logits_jm, logits_jb, logits_mj, logits_mb, logits_bj, logits_bm, \
               pos_mask_jm.detach(), pos_mask_jb.detach(), pos_mask_mj.detach(), \
               pos_mask_mb.detach(), pos_mask_bj.detach(), pos_mask_bm.detach()
