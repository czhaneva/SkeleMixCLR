...
class CrosSCLR_3views_Processor(PT_Processor):
    ...
    def train(self, epoch):
        ...
        for [data1, data2], label in loader:
            ...
            if epoch <= self.arg.cross_epoch:
                ...
                '''
                if self.arg.Avg:
                    loss_jm = - torch.log( (F.softmax(output_jm, dim=1) * mask_jm).sum(1) ) / mask_jm.sum(1)
                    loss_jb = - torch.log( (F.softmax(output_jb, dim=1) * mask_jb).sum(1) ) / mask_jb.sum(1)
                    loss_mj = - torch.log( (F.softmax(output_mj, dim=1) * mask_mj).sum(1) ) / mask_mj.sum(1)
                    loss_mb = - torch.log( (F.softmax(output_mb, dim=1) * mask_mb).sum(1) ) / mask_mb.sum(1)
                    loss_bj = - torch.log( (F.softmax(output_bj, dim=1) * mask_bj).sum(1) ) / mask_bj.sum(1)
                    loss_bm = - torch.log( (F.softmax(output_bm, dim=1) * mask_bm).sum(1) ) / mask_bm.sum(1)
                else:
                    loss_jm = - (F.log_softmax(output_jm, dim=1) * mask_jm).sum(1) / mask_jm.sum(1)
                    loss_jb = - (F.log_softmax(output_jb, dim=1) * mask_jb).sum(1) / mask_jb.sum(1)
                    loss_mj = - (F.log_softmax(output_mj, dim=1) * mask_mj).sum(1) / mask_mj.sum(1)
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--Avg', type=str2bool, default=False, help='use Avg or not')
