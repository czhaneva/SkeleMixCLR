...
class SkeletonCLR_Processor(PT_Processor):
    ...
    def train(self, epoch):
        ...
        for [data1, data2, index], label in loader:
            ...
            # forward
            if epoch <= self.arg.mining_epoch:
                output, pos_mask, output_p_list, pos_mask_p, output_c_list, pos_mask_c, mask, target_mask = \
            ...
            if self.arg.loss == 'ce' or self.arg.loss == 'mnce':
                ...
                loss_reg /= len(output_p_list)
                loss = loss_ins + loss_reg
            elif self.arg.loss == 'dcl':
                loss_ins = self.dcl_loss(output, pos_mask)
                loss_reg = 0
                for output_p, output_c in zip(output_p_list, output_c_list):
                    loss_reg += 0.5 * self.dcl_loss(output_p, pos_mask_p) + \
                                0.5 * self.dcl_loss(output_c, pos_mask_c)
                loss_reg /= len(output_p_list)
                loss = loss_ins + loss_reg
            else:
                raise ValueError
