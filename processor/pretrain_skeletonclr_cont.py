...
class SkeletonCLR_Processor(PT_Processor):
    ...
    def train(self, epoch):
        ...
        for [data1, data2, data3, index], label in loader:
            self.global_step += 1
            # get data
            ...
            # forward
            if epoch <= self.arg.mining_epoch:
                output, pos_mask, output_1, pos_mask_1, mask, target_mask = \
                    self.model(data1, data2, data3, label, topk=0, weight=self.arg.weight)
            else:
                output, pos_mask, output_1, pos_mask_1, mask, target_mask = \
                    self.model(data1, data2, data3, label, topk=self.arg.topk, weight=self.arg.weight)
            ...
            if self.arg.loss == 'ce' or self.arg.loss == 'mnce':
                ...
                loss_reg = self.multi_nce_loss(output_1, pos_mask_1)
                loss = loss_ins + loss_reg
            elif self.arg.loss == 'dcl':
                loss_ins = self.dcl_loss(output, pos_mask)
                loss_reg = self.dcl_loss(output_1, pos_mask_1)
                loss = loss_ins + loss_reg
            else:
                raise ValueError
    ...
    def knn_monitor(self, epoch):
        ...
        if epoch in self.arg.KNN_show:
            try:
                self.KNN_epoch_results[epoch] = acc
            except:
