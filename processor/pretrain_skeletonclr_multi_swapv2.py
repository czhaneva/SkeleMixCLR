...
class SkeletonCLR_Processor(PT_Processor):
    ...
    def train(self, epoch):
        ...
        for [query_list, key, index], label in loader:
            self.global_step += 1
            # get data
            for query_idx in range(len(query_list)):
                query_list[query_idx] = query_list[query_idx].float().cuda(non_blocking=True)
            key = key.float().cuda(non_blocking=True)
            # label = label.long().cuda(non_blocking=True)

            # gen_views
            for query_idx in range(len(query_list)):
                query_list[query_idx] = self.view_gen(query_list[query_idx])
            key = self.view_gen(key)
            ...
            if hasattr(self.model, 'module'):
                self.model.module.update_ptr(output_list[0].size(0))
            else:
                self.model.update_ptr(output_list[0].size(0))
